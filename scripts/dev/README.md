...
|--------|---------|-------------|
| `local_agent.py` | Run an OpenAI Agents SDK agent with a Databricks-hosted model and local MCP | Local + Databricks Model Serving |
...
All MCP tools are available to the model by default. Pass `--tools health,get_current_user` to restrict the agent to a specific subset. The script records MLflow traces by default, including complete agent latency, model spans, tokens, estimated cost, and MCP tool calls. It uses `/Users/<current-user>/mcp-local-agent` unless `--experiment-id`, `--experiment-name`, `MLFLOW_EXPERIMENT_ID`, or `MLFLOW_EXPERIMENT_NAME` selects another experiment. Pass `--no-tracing` to disable it.
